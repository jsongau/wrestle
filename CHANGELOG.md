## 2026-07-30 (Cabinet: Triple H mark redesigned to the King of Kings crown)
- **Triple H's cabinet mark completely redesigned** from a flat 5-point crown to a luxurious gothic "King of Kings" crown: a tall central spire finial, a bold front cross with a glowing ruby at its heart, lateral spikes jutting out either side, and a studded gold band. On hover a gold sheen sweeps across it, the ruby and diamond studs brighten and sparkle in a staggered twinkle, and it lifts with a warm gold glow. Inline in index.html (cabinet FILES array + rfxsig CSS).
- **Fixed hover clip-off** for the mark: scaling now runs from the crown's center (was the base) at a smaller factor with headroom above the finial, plus `overflow:visible`, so the top no longer clips against the tab's rounded corner. Reduced-motion respected; zero console errors.
