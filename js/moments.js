/* Moments hub: facts ticker, discovery console (filter/sort/search), YouTube facades, cinematic hero rotation. */
...
/* moments.js — discovery console for /moments/.
   Filters the .mo-grid of .mo-card by era / type / importance / promotion,
   plus free-text search and sort, with a smooth FLIP reflow. Vanilla, reduced-motion safe.
   The rotating hero + facades/modal come from rankings.js + media.js. */
(function () {
  'use strict';
  var console_ = document.getElementById('mo-console');
  var grid = document.getElementById('mo-grid');
  if (!console_ || !grid) return;
  var cards = [].slice.call(grid.querySelectorAll('.mo-card'));
  var countEl = console_.querySelector('[data-mo-count]');
  var totalEl = console_.querySelector('[data-mo-total]');
  var empty = document.getElementById('mo-empty');
  var searchEl = console_.querySelector('.mo-search');
  var reduce = window.matchMedia && matchMedia('(prefers-reduced-motion:reduce)').matches;
  var state = { era: null, type: null, tier: null, promo: null, sort: 'curated', q: '' };

  var sorters = {
    curated: function (a, b) { return (+a.dataset.idx) - (+b.dataset.idx); },
    newest:  function (a, b) { return (+b.dataset.yr - +a.dataset.yr) || (+a.dataset.idx - +b.dataset.idx); },
    oldest:  function (a, b) { return (+a.dataset.yr - +b.dataset.yr) || (+a.dataset.idx - +b.dataset.idx); },
    az:      function (a, b) { return a.dataset.title < b.dataset.title ? -1 : a.dataset.title > b.dataset.title ? 1 : 0; }
  };
  function matches(el) {
    if (state.era && el.dataset.era !== state.era) return false;
    if (state.type && el.dataset.type !== state.type) return false;
    if (state.tier && el.dataset.tier !== state.tier) return false;
    if (state.promo && el.dataset.promo !== state.promo) return false;
  ...
  function refresh(animate) {
    var first = {};
    if (animate && !reduce) cards.forEach(function (el) { if (!el.hidden) first[el.dataset.idx] = el.getBoundingClientRect(); });
    var ordered = cards.slice().sort(sorters[state.sort] || sorters.curated);
    ordered.forEach(function (el) { grid.appendChild(el); });
    var shown = 0;
    ordered.forEach(function (el) { var ok = matches(el); el.hidden = !ok; if (ok) shown++; });
    ...
    if (animate && !reduce) {
      ...
      cards.forEach(function (el) {
        if (el.hidden) return;
        var f = first[el.dataset.idx]; if (!f) { el.style.transition = 'none'; el.style.opacity = '0'; el.style.transform = 'translateY(10px)'; moved.push([el, true]); return; }
        var l = el.getBoundingClientRect(), dx = f.left - l.left, dy = f.top - l.top;
        if (dx || dy) { el.style.transition = 'none'; el.style.transform = 'translate(' + dx + 'px,' + dy + 'px)'; moved.push([el, false]); }
      });
      requestAnimationFrame(function () {
        moved.forEach(function (m) {
          m[0].style.transition = 'transform .42s cubic-bezier(.22,.61,.36,1),opacity .4s ease';
          m[0].style.transform = ''; m[0].style.opacity = '';
        });
      });
      setTimeout(function () { moved.forEach(function (m) { m[0].style.transition = ''; m[0].style.transform = ''; m[0].style.opacity = ''; m[0].style.willChange = ''; }); grid.classList.remove('is-reflowing'); }, 500);
    }
  }

  function press(sel, active) {
    var chips = console_.querySelectorAll(sel);
    for (var i = 0; i < chips.length; i++) chips[i].setAttribute('aria-pressed', chips[i] === active ? 'true' : 'false');
  }
  console_.addEventListener('click', function (e) {
    var f = e.target.closest('.mo-facet');
    if (f) {
      var g = f.getAttribute('data-group'), v = f.getAttribute('data-f');
      state[g] = (v === 'all') ? null : v;
      press('.mo-facet[data-group="' + g + '"]', f);
      refresh(true); return;
    }
    var s = e.target.closest('.mo-sort');
    if (s) { state.sort = s.getAttribute('data-sort'); press('.mo-sort', s); refresh(true); return; }
  });
  if (searchEl) searchEl.addEventListener('input', function () { state.q = searchEl.value.trim().toLowerCase(); refresh(true); });

  /* era timeline chips filter the console + scroll to it */
  document.addEventListener('click', function (e) {
    var era = e.target.closest('.mo-era[data-era-filter]'); if (!era) return;
    e.preventDefault();
    var v = era.getAttribute('data-era-filter');
    state.era = v;
    press('.mo-facet[data-group="era"]', console_.querySelector('.mo-facet[data-group="era"][data-f="' + v + '"]'));
    refresh(true);
    var top = console_.getBoundingClientRect().top + window.pageYOffset - 80;
    window.scrollTo({ top: top, behavior: 'smooth' });
  });
